...
describe JavaBuildpack::Jre::OpenJDKLikeMemoryCalculator do
  ...
  it 'runs the memory calculator to sanity check',
     cache_fixture: 'stub-memory-calculator.tar.gz',
    ...
    allow(component).to receive(:show_settings).with("#{memory_calculator} -totMemory=$MEMORY_LIMIT" \
                                                      ' -stackThreads=200 -loadedClasses=3')
    component.compile
  end
  ...
  context do
    ...
    let(:configuration) { super().merge 'vm_options' => { 'Xms' => '64m', 'Xmx' => '256G' } }

    it 'create memory calculation command with vm options specified',
       app_fixture: 'jre_memory_calculator_application' do
      ...
      expect(command).to eq('CALCULATED_MEMORY=$($PWD/.java-buildpack/open_jdk_like_memory_calculator/bin/' \
                            ...
                            ' -loadedClasses=3 -vmOptions="-Xms64m -Xmx256G")')
    end
